...
private:
	void BuildLayout();
	void BuildCreateDialog(UCanvasPanel* Root);
	void BuildPersonQuizSelectionControls();
	void HandleRoomsFound(bool bSuccess, const TArray<FIdolQuizRoomInfo>& Rooms);
	void HandleSessionAction(bool bSuccess, const FString& Message);
	void JoinIndex(int32 Index);
	void RefreshGameSettingsVisibility();
	void HandlePoolCheckChanged(int32 PoolIndex, bool bChecked);
	void SelectQuestionCount(int32 QuestionCount);
	void RefreshQuestionCountButtons();
	FString BuildSelectedPoolTags() const;
	int32 GetSelectedPoolCapacity() const;

	UFUNCTION() void OpenCreateDialog();
	UFUNCTION() void CloseCreateDialog();
	UFUNCTION() void ConfirmCreateRoom();
	UFUNCTION() void HandleGameChanged(FString SelectedItem, ESelectInfo::Type SelectionType);
	UFUNCTION() void HandleCategoryChanged(FString SelectedItem, ESelectInfo::Type SelectionType);
	UFUNCTION() void HandleIdolChecked(bool bChecked);
	UFUNCTION() void HandleActorChecked(bool bChecked);
	UFUNCTION() void HandleOnePieceChecked(bool bChecked);
	...
	UFUNCTION() void Select200Questions();
	UFUNCTION() void Select250Questions();
	UFUNCTION() void Select300Questions();
	UFUNCTION() void HandleRefresh();
	UFUNCTION() void HandleMainHub();
	UFUNCTION() void Join0();
	UFUNCTION() void Join1();
	UFUNCTION() void Join2();
	UFUNCTION() void Join3();
	UFUNCTION() void Join4();
	UFUNCTION() void Join5();
	...
	UPROPERTY(Transient) TObjectPtr<UEditableTextBox> RoomNameInput;
	UPROPERTY(Transient) TObjectPtr<UComboBoxString> GameDropdown;
	UPROPERTY(Transient) TObjectPtr<UComboBoxString> CategoryDropdown;
	UPROPERTY(Transient) TObjectPtr<UComboBoxString> QuestionCountDropdown;
	UPROPERTY(Transient) TObjectPtr<UComboBoxString> DrawingRoundsDropdown;
	UPROPERTY(Transient) TObjectPtr<UComboBoxString> DrawingTimeDropdown;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> CategoryLabel;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> QuestionCountLabel;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> DrawingRoundsLabel;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> DrawingTimeLabel;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> DialogErrorText;
	UPROPERTY(Transient) TObjectPtr<UTextBlock> StatusText;
	UPROPERTY(Transient) TArray<TObjectPtr<UCheckBox>> PoolCheckBoxes;
	UPROPERTY(Transient) TArray<TObjectPtr<UButton>> QuestionCountButtons;
	UPROPERTY(Transient) TArray<TObjectPtr<UButton>> RoomButtons;
	UPROPERTY(Transient) TArray<TObjectPtr<UTextBlock>> RoomLabels;
	UPROPERTY(Transient) TObjectPtr<UIdolQuizSessionSubsystem> Sessions;

	EIdolQuizRoomCategory SelectedCategory = EIdolQuizRoomCategory::Idol;
	EMiniGameRoomType SelectedGameType = EMiniGameRoomType::PersonQuiz;
	int32 SelectedQuestionCount = 50;

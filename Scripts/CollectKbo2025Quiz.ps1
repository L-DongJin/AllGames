param(
    [string]$DatasetRoot = "SourceAssets/IdolQuiz/KBO2025",
    [int]$ExpectedCandidates = 256,
    [int]$CheckpointSize = 10,
    [int]$DelayMilliseconds = 750
)

$ErrorActionPreference = "Stop"
$sourcePage = "https://www.piku.co.kr/w/73uvgh"
$rankingPage = "https://www.piku.co.kr/w/rank/73uvgh"
$rankingEndpoint = "https://www.piku.co.kr/w/rank/x.php?u=73uvgh"
...
if (Test-Path -LiteralPath $manifestPath) {
    ...
    $duplicateFilenameGroups = @($records | Group-Object { $_.saved_filename } | Where-Object Count -gt 1)
    foreach ($group in $duplicateFilenameGroups) {
        $duplicateIndex = 1
        foreach ($record in $group.Group) {
            $baseName = [IO.Path]::GetFileNameWithoutExtension($record.saved_filename)
            $record.saved_filename = "$baseName`_$('{0:D3}' -f $duplicateIndex).png"
            $duplicateIndex++
        }
    }

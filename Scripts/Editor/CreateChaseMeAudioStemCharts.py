...
MUSIC_PATH = "/Game/Audio/Music/CHASE-ME"
RECORDING_DIR = "CHASEMEAudioStemDifficulties"
DIFFICULTIES = [
...
def main():
    ...
    if catalog is None or music is None:
        raise RuntimeError("CHASE-ME catalog or master music asset is missing")

    base_dir = os.path.join(unreal.Paths.project_saved_dir(), "ChartRecordings", RECORDING_DIR)
    with open(os.path.join(base_dir, "CHASEME_metadata.json"), "r", encoding="utf-8") as stream:
        metadata = json.load(stream)

    new_charts = []
    for difficulty_name, difficulty in DIFFICULTIES:
        csv_path = os.path.join(base_dir, "CHASEME_{}_5Key.csv".format(difficulty_name))
        with open(csv_path, "r", encoding="utf-8", newline="") as stream:
            rows = list(csv.DictReader(stream))

        asset_name = "DA_CHASEME_{}_5Key".format(difficulty_name)
        ...
        for row in rows:
            ...
            note.set_editor_property("duration_seconds", float(row["duration_seconds"]))
            note.set_editor_property("lane_index", int(row["lane_index"]))
            notes.append(note)

        asset.set_editor_property("song_title", "CHASE-ME")
        asset.set_editor_property("music", music)
        asset.set_editor_property("preview_start_time_seconds", 45.0)
    ...
    existing = [
        ...
        if chart and not chart.get_name().startswith("DA_CHASEME_")
    ]
    ...
    unreal.log("CHASE-ME AUDIO-STEM CHARTS READY: {}".format(
